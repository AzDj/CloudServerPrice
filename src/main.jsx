...
const 服务器规格精确云商 = {
  '1C2G': ['aliyun', 'tencent', 'huawei', 'aws', 'azure', 'gcp'],
  '2C4G': ['aliyun', 'tencent', 'huawei', 'aws', 'azure', 'gcp'],
  '4C8G': ['aliyun', 'tencent', 'huawei', 'aws', 'azure', 'gcp'],
...
const RDS规格档位 = {
  '1C2G / 50GB': {
    ...
    memory: 2,
    storage: '50GB',
    月价: { aliyun: 168.00, tencent: 158.00, huawei: 152.00, aws: 245.00, azure: 256.00, gcp: 238.00 },
  },
  '2C4G / 100GB': {
    cpu: 2,
    memory: 4,
    storage: '100GB',
    月价: { aliyun: 298.00, tencent: 286.00, huawei: 279.00, aws: 430.00, azure: 448.00, gcp: 418.00 },
    存储覆盖: { azure: '128GB', gcp: '128GB' },
  },
  '4C8G / 200GB': {
    cpu: 4,
    memory: 8,
    storage: '200GB',
    月价: { aliyun: 560.00, tencent: 538.00, huawei: 516.00, aws: 790.00, azure: 824.00, gcp: 758.00 },
  },
  '4C16G / 500GB': {
    cpu: 4,
    memory: 16,
    storage: '500GB',
    月价: { aliyun: 840.00, tencent: 812.00, huawei: 786.00, aws: 1180.00, azure: 1240.00, gcp: 1128.00 },
  },
  '8C16G / 500GB': {
    cpu: 8,
    memory: 16,
    storage: '500GB',
    月价: { aliyun: 1098.00, tencent: 1056.00, huawei: 1018.00, aws: 1560.00, azure: 1628.00, gcp: 1498.00 },
  },
  '8C32G / 1TB': {
    cpu: 8,
    memory: 32,
    storage: '1TB',
    月价: { aliyun: 1580.00, tencent: 1518.00, huawei: 1466.00, aws: 2260.00, azure: 2388.00, gcp: 2180.00 },
  },
};

const RDS实例规格 = {
  aliyun: ['mysql.n1.small.1', 'mysql.n2.medium.1', 'mysql.n4.large.1', 'mysql.n4.large.2', 'mysql.n8.xlarge.1', 'mysql.n8.xlarge.2'],
  tencent: ['MYSQL.S1.SMALL2', 'MYSQL.S2.MEDIUM4', 'MYSQL.S4.LARGE8', 'MYSQL.S4.LARGE16', 'MYSQL.S8.XLARGE16', 'MYSQL.S8.XLARGE32'],
  huawei: ['rds.mysql.c6.medium.2', 'rds.mysql.c6.large.2', 'rds.mysql.c6.xlarge.2', 'rds.mysql.c6.xlarge.4', 'rds.mysql.c6.2xlarge.2', 'rds.mysql.c6.2xlarge.4'],
  aws: ['db.t4g.small', 'db.t4g.medium', 'db.m7g.large', 'db.m7g.xlarge', 'db.m7g.2xlarge', 'db.m7g.2xlarge'],
  azure: ['B1ms Flexible', 'B2ms Flexible', 'D4as v5 Flexible', 'D4s v5 Flexible', 'D8as v5 Flexible', 'D8s v5 Flexible'],
  gcp: ['db-custom-1-2048', 'db-custom-2-4096', 'db-custom-4-8192', 'db-custom-4-16384', 'db-custom-8-16384', 'db-custom-8-32768'],
};

const 规格筛选配置 = {
  全部: { 标签: '规格筛选', 选项: ['默认规格'], 默认值: '默认规格', 禁用: true },
  服务器: { 标签: '服务器规格', 选项: 服务器规格选项, 默认值: '2C4G', 禁用: false },
  数据库: { 标签: 'RDS规格', 选项: RDS规格选项, 默认值: '2C4G / 100GB', 禁用: false },
  存储: { 标签: '规格筛选', 选项: ['不适用'], 默认值: '不适用', 禁用: true },
  网络: { 标签: '规格筛选', 选项: ['不适用'], 默认值: '不适用', 禁用: true },
};
  ...
  },
  {
    ...
    行: [
      ...
      },
      {
        ...
        对齐: 'text',
        值: {
          aliyun: { 文本: '静态资源 / HTTPS / 图片小文件', 标签: ['静态'] },
          tencent: { 文本: '静态资源 / HTTPS / 网页小文件', 标签: ['静态'] },
          huawei: { 文本: '静态资源 / HTTPS / 防盗链', 标签: ['低价'] },
          aws: { 文本: 'CloudFront / 静态内容分发 / HTTPS', 标签: ['全球'] },
          azure: { 文本: 'Azure CDN / 静态内容分发 / HTTPS', 标签: ['静态'] },
          gcp: { 文本: 'Cloud CDN / 静态缓存 / HTTPS', 标签: ['边缘'] },
        },
      },
      { 维度: '地域', 对齐: 'text', 动态: 'region' },
      { 维度: '计费方式', 对齐: 'text', 动态: 'billing' },
...
function 构建RDS模块(模块, RDS规格) {
  if (模块.id !== 'database') {
    return 模块;
  }

  const 档位 = RDS规格档位[RDS规格] || RDS规格档位['2C4G / 100GB'];
  const 规格索引 = RDS规格选项.indexOf(RDS规格) >= 0 ? RDS规格选项.indexOf(RDS规格) : 1;
  const 指标 = 云商列表.reduce((结果, 云商) => {
    const 月价 = 档位.月价[云商.id];
    结果[云商.id] = {
      ...模块.指标[云商.id],
  ...
  const 更新行 = 模块.行.map((行) => {
    ...
    if (行.维度 === '规格补充') {
      return {
        ...行,
        值: {
          aliyun: { 文本: `MySQL 8.0 / 高可用 / ESSD ${档位.storage}`, 标签: ['主备'] },
          tencent: { 文本: `MySQL 8.0 / 双节点 / SSD ${档位.storage}`, 标签: ['主备'] },
          huawei: { 文本: `MySQL 8.0 / 主备 / SSD ${档位.storage}`, 标签: ['主备'] },
          aws: { 文本: `RDS MySQL / Multi-AZ 可选 / ${档位.存储覆盖?.aws || 档位.storage}`, 标签: ['全球'] },
          azure: { 文本: `Flexible Server / Zone Redundant / ${档位.存储覆盖?.azure || 档位.storage}`, 标签: ['高 SLA'] },
          gcp: { 文本: `Cloud SQL / HA 可选 / ${档位.存储覆盖?.gcp || 档位.storage}`, 标签: ['自动备份'] },
        },
      };
    }
    if (行.维度 === '按量单价') {
  ...
  return { ...模块, 指标, 行: 更新行, RDS规格 };
}

function 构建资源模块(模块, 状态) {
  if (模块.id === 'compute') {
    return 构建云服务器模块(模块, 状态.规格筛选);
  }
  if (模块.id === 'database') {
    return 构建RDS模块(模块, 状态.规格筛选);
  }
  return 模块;
}
...
function 是精确规格匹配(模块, 云商, 状态) {
  if (模块.id === 'compute') {
    return 服务器规格精确云商[状态.规格筛选]?.includes(云商.id) ?? true;
  }
  return true;
}

function 是RDS存储精确匹配(模块, 云商) {
  if (模块.id !== 'database' || !模块.RDS规格) {
    return true;
  }
  const 档位 = RDS规格档位[模块.RDS规格];
...
function 获取单元格值(模块, 行, 云商, 状态) {
  ...
  const 值 = 行.值[云商.id];
  const 是规格行 =
    行.维度 === 'CPU / 内存' && ['compute', 'database'].includes(模块.id);
  const 是精确规格 = 是精确规格匹配(模块, 云商, 状态);
  if (是规格行 && !是精确规格) {
    return 合并单元格提示(值, ['无对应CPU/内存'], true);
  }
  return 模块.id === 'database' && 行.维度 === '规格补充' && !是RDS存储精确匹配(模块, 云商)
    ? 合并单元格提示(值, ['无对应存储容量'], true)
    : 值;
}
...
function 获取规格筛选配置(资源类型) {
  return 规格筛选配置[资源类型] || 规格筛选配置.全部;
}
...
function Controls({ 状态, 设置状态 }) {
  ...
  return (
    <div className="controls" aria-label="筛选与排序">
      ...
      <SelectControl
        标签={当前规格配置.标签}
        值={当前规格配置.选项.includes(状态.规格筛选) ? 状态.规格筛选 : 当前规格配置.默认值}
        选项={当前规格配置.选项}
        disabled={当前规格配置.禁用}
        onChange={(值) => 设置状态((旧) => ({ ...旧, 规格筛选: 值 }))}
      />
      <SelectControl
        标签="币种"
        值={状态.币种}
...
function SelectControl({ 标签, 值, 选项, onChange, disabled = false }) {
  return (
    <label className="select-control">
      ...
      <select
        value={值}
        disabled={disabled}
        onChange={(事件) => onChange(事件.target.value)}
      >
        {选项.map((选项值) => (
...
function 获取标签类名(标签) {
  ...
  if (标签.includes('就近') || 标签.includes('非筛选') || 标签.includes('无对应')) {
